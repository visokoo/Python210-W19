...
'''
----------------------------------------------------
Assignment: mailroom-p3.py
Author: visokoo | Created: 2/1/2019
ChangeLog: 2/10/2019, Added error handling
...
Pt.3
- Add exceptions and error handling to your code
----------------------------------------------------
'''
...
def find_donor(donor_name):
  [donor_name if row["name"].lower() == donor_name.lower() else donor_name for row in donors]
  return donor_name
...
def thank_you():
  ...
  except ValueError: raise Exception("The donation amount must be a number, try again.")
...
def generate_email(donor_name, donation):
  email = "Dear {}, \n\nThank you so much for your generous donation amount of ${:.2f}.\n" \
          ...
          "Cheers,\n" \
          "A grateful volunteer".format(donor_name.split(' ')[0].capitalize(), donation)
  return email
...
def generate_report():
  ...
  for row in sorted_donors:
    print("{:25}| ${:>15.2f} | {:>11} | ${:11.2f}".format(row["name"], row["total_donation"], row["total_donation_count"], row["total_donation"] / row["total_donation_count"]))
  print()
